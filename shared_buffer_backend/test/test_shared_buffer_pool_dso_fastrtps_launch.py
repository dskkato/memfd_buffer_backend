...
def generate_test_description():
    """Verify shared_buffer pool sharing between a component and the backend plugin."""
    test_domain_id = str(100 + os.getpid() % 100)
    subscriber_container = ComposableNodeContainer(
        name='shared_buffer_image_dso_subscriber_container',
        namespace='',
        ...
        composable_node_descriptions=[
            ComposableNode(
                ...
                name='shared_buffer_image_subscriber',
            ),
        ],
        output='screen',
    )
    ...
    publisher_container = ComposableNodeContainer(
        ...
        composable_node_descriptions=[
            ComposableNode(
                package='shared_buffer_backend',
                plugin='SharedImageDsoPublisher',
                name='shared_buffer_image_dso_publisher',
            ),
        ],
        output='screen',
    )
...
class TestSharedBufferPoolDsoFastRTPS(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.node.create_subscription(
            UInt32, 'shared_buffer_dso_subscriber_count', self._count_cb, 10)
        self.node.create_subscription(
            Bool, 'shared_buffer_dso_validation', self._validation_cb, 10)
        self.node.create_subscription(
            Bool, 'shared_buffer_dso_backend_validation', self._backend_cb, 10)
        self.node.create_subscription(
    ...
    def test_five_shared_buffer_messages_validate(self):
        """Require explicit shared_buffer, metadata, and payload validation."""
        ...
        self.assertTrue(self.all_valid, 'At least one shared_buffer DSO validation failed')


@launch_testing.post_shutdown_test()
class TestSharedBufferPoolDsoFastRTPSShutdown(unittest.TestCase):
