...
#ifndef SHARED_BUFFER__SHARED_BUFFER_API_HPP_
#define SHARED_BUFFER__SHARED_BUFFER_API_HPP_
...
/// \brief Allocate a fresh shared-memory-backed \c rosidl::Buffer<std::uint8_t>.
///
/// \details Allocates \p count payload bytes from the shared buffer memory pool.
...
{
  return rosidl::Buffer<std::uint8_t>(std::make_unique<SharedBufferImpl<std::uint8_t>>(count));
}

namespace detail
{

/// \brief Allocate a shared, fresh shared-memory-backed \c rosidl::Buffer<std::uint8_t>.
inline std::shared_ptr<rosidl::Buffer<std::uint8_t>> allocate_shared_buffer_shared(
  std::size_t byte_count)
{
  return std::make_shared<rosidl::Buffer<std::uint8_t>>(
    std::make_unique<SharedBufferImpl<std::uint8_t>>(byte_count));
}

template<typename T>
inline SharedBufferImpl<T> * shared_buffer_impl_of(rosidl::Buffer<T> & buffer)
{
...
WriteHandle from_output_buffer(rosidl::Buffer<T> & buffer)
{
  ...
  if (impl == nullptr) {
    throw SharedBufferError("from_output_buffer called with null buffer implementation");
  }
  if (buffer.size() == 0) {
    throw SharedBufferError("from_output_buffer called on empty buffer");
  }
...
/// is already shared-memory-backed, a read handle for that allocation is returned directly.
/// Otherwise, a fresh shared-memory-backed \c rosidl::Buffer<std::uint8_t> of the same byte
...
ReadHandle from_input_buffer(const rosidl::Buffer<T> & buffer)
{
  ...
  if (impl == nullptr) {
    throw SharedBufferError("from_input_buffer called with null buffer implementation");
  }
  if (buffer.size() == 0) {
    throw SharedBufferError("from_input_buffer called on empty buffer");
  }

  if (const auto * shared_buffer_impl = dynamic_cast<const SharedBufferImpl<T> *>(impl)) {
    return shared_buffer_impl->get_shared_buffer().get_read_handle();
  }

  if (buffer.size() > std::numeric_limits<std::size_t>::max() / sizeof(T)) {
    throw SharedBufferError("input buffer size overflows size_t");
  }
  ...
  auto read = promoted_impl->get_shared_buffer().get_read_handle();
...
#endif  // SHARED_BUFFER__SHARED_BUFFER_API_HPP_
