...
flowchart LR
  ...
  Plugin -.->|"platform IPC transport"| Broker[MemfdFdBroker]
...
- `MemfdFdBroker` exposes the block's platform IPC identity. On Linux it uses a
...
On Windows no listener broker is needed. `memfd_socket_path` carries an ASCII
...
A relay can subscribe to a memfd-backed message and publish the same message
without copying its payload. The important distinction is that the relay's
...
reader reference held by the imported `MemfdBuffer` prevents the source pool
...
`MemfdImageRelay` demonstrates the normal callback-scoped case.
