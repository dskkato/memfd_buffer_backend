...
int create_memfd()
{
  return static_cast<int>(syscall(SYS_memfd_create, "rosidl_memfd_buffer", MFD_CLOEXEC));
}
...
{
  if (block == nullptr || block->memfd < 0 || impl_->dispatcher == nullptr) {
    return {};
  }
  std::lock_guard<std::mutex> lock(impl_->mutex);
  ...
  std::size_t mapped_size, std::uint32_t, std::uint64_t)
{
  ...
  return MemfdPlatformMapping{fd, mapping, mapped_size, {}};
}
...
MemfdPlatformMapping import_platform_mapping(const std::string & ipc_name, std::size_t mapped_size)
{
  ...
  return MemfdPlatformMapping{fd, mapping, mapped_size, ipc_name};
}
