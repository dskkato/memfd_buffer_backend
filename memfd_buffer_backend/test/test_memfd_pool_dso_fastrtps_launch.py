...
def generate_test_description():
    ...
    relay_container = ComposableNodeContainer(
        name='memfd_image_dso_relay_container',
        namespace='',
        package='rclcpp_components',
        executable='component_container',
        composable_node_descriptions=[
            ComposableNode(
                package='memfd_buffer_backend',
                plugin='MemfdImageRelay',
                name='memfd_image_relay',
                parameters=[{
    ...
    relay_subscriber_container = ComposableNodeContainer(
        name='memfd_image_dso_relay_subscriber_container',
        namespace='',
        package='rclcpp_components',
        executable='component_container',
        composable_node_descriptions=[
            ComposableNode(
                package='memfd_buffer_backend',
                plugin='MemfdImageSubscriber',
                name='memfd_image_relay_subscriber',
                parameters=[{
                    'input_topic': 'test_memfd_image_dso_relay',
                    'result_prefix': 'memfd_relay_dso',
                }],
            ),
        ],
        output='screen',
...
class TestMemfdPoolDsoFastRTPS(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.node.create_subscription(
            UInt32, 'memfd_relay_dso_subscriber_count', self._relay_count_cb, 10)
        self.node.create_subscription(
            Bool, 'memfd_relay_dso_validation', self._validation_cb, 10)
        self.node.create_subscription(
            Bool, 'memfd_relay_dso_backend_validation', self._backend_cb, 10)
        self.node.create_subscription(
            Bool, 'memfd_relay_dso_content_validation', self._content_cb, 10)
        self.node.create_subscription(
            Bool, 'memfd_relay_dso_metadata_validation', self._metadata_cb, 10)
