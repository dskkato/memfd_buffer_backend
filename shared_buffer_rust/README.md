...
view) -> InitializedWriteAccess ([u8] view) -> Buffer -> ReadAccess`. The
native write lease is one-shot: dropping or finishing it finalizes the buffer,
and no later write access can be acquired. `write_from_slice` validates the
length before touching the payload; on mismatch its error can return the write
access for a retry. Use `write_from_slice` or `fill`, followed by `finish`, for
safe initialization; `assume_init` is available for callers that initialized
every byte manually and is `unsafe`.
